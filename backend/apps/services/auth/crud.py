...
from apps.services.system import crud as system_crud
...
class UserDal(DalBase):
    ...
    async def recursion_get_dept_ids(
        ...
        """
        if not depts:
            depts = await DeptDal(self.db).get_datas(limit=0, return_objs=True)
            result = []
            for i in user.depts:
                result.append(i.id)
    ...
    async def create_data(
        ...
        if data.role_ids:
            roles = await RoleDal(self.db).get_datas(limit=0, id=("in", data.role_ids), return_objs=True)
            for role in roles:
                obj.roles.add(role)
        if data.dept_ids:
            depts = await DeptDal(self.db).get_datas(limit=0, id=("in", data.dept_ids), return_objs=True)
            for dept in depts:
                obj.depts.add(dept)
    ...
    async def put_data(
        ...
        :return:
        """
        obj = await self.get_data(data_id, options=[joinedload(self.model.roles)])
        data_dict = jsonable_encoder(data)
        for key, value in data_dict.items():
            if key == "role_ids":
                if value:
                    roles = await RoleDal(self.db).get_datas(limit=0, id=("in", value), return_objs=True)
                    if obj.roles:
                        obj.roles.clear()
                    for role in roles:
                        obj.roles.add(role)
                ...
                if value:
                    depts = await DeptDal(self.db).get_datas(limit=0, id=("in", value), return_objs=True)
                    if obj.depts:
                        obj.depts.clear()
                    for dept in depts:
                        obj.depts.add(dept)
    ...
    async def export_query_list(self, header: list, params: UserParams) -> dict:
        ...
        rows = []
        options = await system_crud.DictTypeDal(self.db).get_dicts_details(["sys_gender"])
        for user in datas:
            data = []
            for item in header:
    ...
    async def get_import_headers_options(self) -> None:
        ...
        # 性别选择项
        dict_types = await system_crud.DictTypeDal(self.db).get_dicts_details(["sys_gender"])
    ...
    async def delete_datas(self, ids: list[int], soft: bool = False, **kwargs) -> None:
        ...
        objs = await self.get_datas(limit=0, id=("in", ids), options=options, return_objs=True)
        for obj in objs:
            if obj.roles:
                obj.roles.clear()
        return await super(UserDal, self).delete_datas(ids, soft, **kwargs)
...
class RoleDal(DalBase):
    ...
    async def create_data(
        ...
        if data.menu_ids:
            menus = await MenuDal(db=self.db).get_datas(limit=0, id=("in", data.menu_ids), return_objs=True)
            for menu in menus:
                obj.menus.add(menu)
        if data.dept_ids:
            depts = await DeptDal(db=self.db).get_datas(limit=0, id=("in", data.dept_ids), return_objs=True)
            for dept in depts:
                obj.depts.add(dept)
        await self.flush(obj)
        return await self.out_dict(obj, options, return_obj, schema)
    ...
    async def put_data(
        ...
        :return:
        """
        obj = await self.get_data(data_id, options=[joinedload(self.model.menus), joinedload(self.model.depts)])
        obj_dict = jsonable_encoder(data)
        for key, value in obj_dict.items():
            if key == "menu_ids":
                if value:
                    menus = await MenuDal(db=self.db).get_datas(limit=0, id=("in", value), return_objs=True)
                    if obj.menus:
                        obj.menus.clear()
                    for menu in menus:
                        obj.menus.add(menu)
                ...
                if value:
                    depts = await DeptDal(db=self.db).get_datas(limit=0, id=("in", value), return_objs=True)
                    if obj.depts:
                        obj.depts.clear()
                    for dept in depts:
                        obj.depts.add(dept)
    ...
    async def get_role_menu_tree(self, role_id: int) -> list:
        role = await self.get_data(role_id, options=[joinedload(self.model.menus)])
    ...
    async def delete_datas(self, ids: list[int], soft: bool = False, **kwargs) -> None:
        ...
        :param kwargs: 其他更新字段
        """
        user_count = await UserDal(self.db).get_count(join=[["roles"]], where=[models.Role.id.in_(ids)])
        if user_count > 0:
            raise CustomException("无法删除存在用户关联的角色", code=400)
        return await super(RoleDal, self).delete_datas(ids, soft, **kwargs)
...
class MenuDal(DalBase):
    ...
    async def get_routers(self, user: models.User) -> list:
        ...
        if any([i.is_admin for i in user.roles]):
            ...
            user = await UserDal(self.db).get_data(user.id, options=options)
            datas = set()
    ...
    async def delete_datas(self, ids: list[int], soft: bool = False, **kwargs) -> None:
        ...
        :return:
        """
        count = await RoleDal(self.db).get_count(join=[["menus"]], where=[self.model.id.in_(ids)])
        if count > 0:
            raise CustomException("无法删除存在角色关联的菜单", code=400)
        await super(MenuDal, self).delete_datas(ids, soft, **kwargs)
...
class TestDal(DalBase):
    ...
    def __init__(self, db: AsyncSession):
        super(TestDal, self).__init__(db, models.User, schemas.UserSimpleOut)

    async def test(self):
        # print("-----------------------开始------------------------")
        options = [joinedload(self.model.roles)]
        join = [[self.model.roles]]
        where = [self.model.id == 1, models.Role.id == 1]
        start_sql = select(self.model)
