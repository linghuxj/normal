...
from .m_role import Role
from .m_m2m import auth_user_roles, auth_user_depts
...
class User(BaseModel):
    """
    用户表: 用户基本信息
    包含: 头像, 手机号, 邮箱地址, 姓名, 昵称, 密码, 性别, 是否可用, 是否已经重置密码，没有重置的，登陆系统后必须重置密码,
    最后一次登录IP, 最近一次登录时间, 是否为工作人员, 服务端微信平台openid, 是否已有服务端微信平台openid
    """
