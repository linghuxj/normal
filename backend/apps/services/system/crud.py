...
class SettingsDal(DalBase):
    ...
    async def get_base_config(self) -> dict:
        ...
        datas = await self.get_datas(limit=0, tab_id=("in", ["1", "9"]), disabled=False, return_objs=True)
        result = {}
        for config in datas:
...
class TaskDal(MongoManage):
    ...
    async def get_task(
        ...
        :param _id: 数据 ID
        :param return_none: 是否返回空 None，否则抛出异常，默认抛出异常
        ...
        data = data[0]
        if data and schema:
            return jsonable_encoder(schema(**data))
        return data
    ...
    async def get_tasks(
        ...
        last_run_datetime: 在 scheduler_task_record 中获取该任务最近一次执行完成的时间
        """
        order_field = order_field if order_field else 'create_datetime'
        order = -1 if order in self.ORDER_FIELD else 1
        params = self.filter_condition(**kwargs)
        pipeline = [
            {
                '$addFields': {
        ...
        if count == 0:
            ...
            datas = [jsonable_encoder(schema(**data)) for data in datas]
        elif self.schema:
    ...
    async def create_task(self, rd: Redis, data: schemas.Task) -> dict:
        ...
        obj = await self.get_task(insert_result.inserted_id, schema=schemas.TaskSimpleOut)

        # 如果分组不存在则新增分组
        group = await TaskGroupDal(self.db).get_data(value=data.group, return_none=True)
        if not group:
    ...
    async def put_task(self, rd: Redis, _id: str, data: schemas.Task) -> dict:
        ...
        obj: dict = await self.get_task(_id, schema=schemas.TaskSimpleOut)
    ...
    async def run_once_task(self, rd: Redis, _id: str) -> int:
        ...
        执行一次任务
        """
        obj: dict = await self.get_data(_id, schema=schemas.TaskSimpleOut)
        message = {
            "operation": self.JobOperation.add.value,
            "task": {
                "exec_strategy": "once",
