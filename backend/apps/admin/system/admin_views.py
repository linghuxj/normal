...
from apps.services.system import schemas, crud
from core.dependencies import IdList
from apps.services.auth.utils.current import AllUserAuth, FullAdminAuth, OpenAuth
from apps.services.auth.utils.validation.auth import Auth
from apps.services.system.params import DictTypeParams, DictDetailParams, TaskParams, TaskRecordParams
from apps.services.auth import crud as auth_crud
...
async def get_dict_types(p: DictTypeParams = Depends(), auth: Auth = Depends(AllUserAuth())):
    datas, count = await crud.DictTypeDal(auth.db).get_datas(**p.dict(), return_count=True)
...
async def get_dict_type(data_id: int, auth: Auth = Depends(AllUserAuth())):
    ...
    return SuccessResponse(await crud.DictTypeDal(auth.db).get_data(data_id, schema=schema))
...
async def get_dict_details(params: DictDetailParams = Depends(), auth: Auth = Depends(AllUserAuth())):
    datas, count = await crud.DictDetailsDal(auth.db).get_datas(**params.dict(), return_count=True)
    return SuccessResponse(datas, count=count)
...
async def delete_dict_details(ids: IdList = Depends(), auth: Auth = Depends(AllUserAuth())):
    await crud.DictDetailsDal(auth.db).delete_datas(ids.ids, soft=False)
    return SuccessResponse("删除成功")
...
async def get_dict_detail(data_id: int, auth: Auth = Depends(AllUserAuth())):
    ...
    return SuccessResponse(await crud.DictDetailsDal(auth.db).get_data(data_id, schema=schema))
...
async def sms_send(telephone: str, rd: Redis = Depends(redis_getter), auth: Auth = Depends(OpenAuth())):
    user = await auth_crud.UserDal(auth.db).get_data(telephone=telephone, return_none=True)
    if not user:
...
async def get_task(
    ...
    return SuccessResponse(await crud.TaskDal(db).get_task(_id, schema=schemas.TaskSimpleOut))
