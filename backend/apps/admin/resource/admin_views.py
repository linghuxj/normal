...
from apps.services.resource import schemas, crud, params, models
...
async def delete_images(ids: IdList = Depends(), auth: Auth = Depends(FullAdminAuth())):
    await crud.ImagesDal(auth.db).delete_datas(ids.ids, soft=False)
    return SuccessResponse("删除成功")


@app.get("/images/{data_id}", summary="获取图片信息")
async def get_images(data_id: int, auth: Auth = Depends(FullAdminAuth())):
    return SuccessResponse(await crud.ImagesDal(auth.db).get_data(data_id, schema=schemas.ImagesSimpleOut))
