...
from apps.services.help import schemas, crud, params, models
...
async def get_issue_categorys_options(auth: Auth = Depends(AllUserAuth())):
    ...
    return SuccessResponse(await crud.IssueCategoryDal(auth.db).get_datas(limit=0, is_active=True, schema=schema))
...
async def delete_issue_categorys(ids: IdList = Depends(), auth: Auth = Depends(AllUserAuth())):
    await crud.IssueCategoryDal(auth.db).delete_datas(ids=ids.ids, soft=False)
...
async def get_issue_category(data_id: int, auth: Auth = Depends(AllUserAuth())):
    ...
    return SuccessResponse(await crud.IssueCategoryDal(auth.db).get_data(data_id, schema=schema))
...
async def delete_issues(ids: IdList = Depends(), auth: Auth = Depends(AllUserAuth())):
    await crud.IssueDal(auth.db).delete_datas(ids=ids.ids, soft=False)
    return SuccessResponse("删除成功")
...
async def get_issue(data_id: int, db: AsyncSession = Depends(db_getter)):
    ...
    return SuccessResponse(await crud.IssueDal(db).get_data(data_id, schema=schema))
