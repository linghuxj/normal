...
from apps.services.auth import schemas, crud, models
from apps.services.auth.params import UserParams, RoleParams, DeptParams
from apps.services.auth.utils.current import AllUserAuth, FullAdminAuth
...
async def delete_users(ids: IdList = Depends(), auth: Auth = Depends(FullAdminAuth(permissions=["auth.user.delete"]))):
    ...
    await crud.UserDal(auth.db).delete_datas(ids=ids.ids, soft=True, is_active=False)
...
async def get_user(
    ...
    options = [joinedload(model.roles), joinedload(model.depts)]
    schema = schemas.UserOut
    return SuccessResponse(await crud.UserDal(auth.db).get_data(data_id, options=options, schema=schema))
...
async def get_roles(
    ...
    datas, count = await crud.RoleDal(auth.db).get_datas(**params.dict(), return_count=True)
    return SuccessResponse(datas, count=count)
...
async def delete_roles(ids: IdList = Depends(), auth: Auth = Depends(FullAdminAuth(permissions=["auth.role.delete"]))):
    ...
    await crud.RoleDal(auth.db).delete_datas(ids.ids, soft=False)
    return SuccessResponse("删除成功")
...
async def get_role(
    ...
    return SuccessResponse(await crud.RoleDal(auth.db).get_data(data_id, options=options, schema=schema))
...
async def delete_menus(ids: IdList = Depends(), auth: Auth = Depends(FullAdminAuth(permissions=["auth.menu.delete"]))):
    await crud.MenuDal(auth.db).delete_datas(ids.ids, soft=False)
    return SuccessResponse("删除成功")
...
async def put_menus(
    ...
    return SuccessResponse(await crud.MenuDal(auth.db).get_data(data_id, schema=schema))
...
async def delete_depts(ids: IdList = Depends(), auth: Auth = Depends(FullAdminAuth())):
    await crud.DeptDal(auth.db).delete_datas(ids.ids, soft=False)
    return SuccessResponse("删除成功")
