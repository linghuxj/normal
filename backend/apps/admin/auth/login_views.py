...
from apps.services.auth.utils.login_manage import LoginManage
from apps.services.auth.utils.validation import LoginForm, WXLoginForm
from apps.services.auth.utils.current import FullAdminAuth
from apps.services.auth.utils.validation.auth import Auth
from apps.services.record.models import LoginRecord
from apps.services.auth.crud import MenuDal, UserDal
...
async def api_login_for_access_token(
    ...
    user = await UserDal(db).get_data(telephone=data.username, return_none=True)
    if not user:
